...
const SingleTripModal = ({ id, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchTrip = async () => {
      ...
      setStoreOrdersCount(
        Object.entries(storeCountMap).map(([storeName, count]) => ({ storeName, count }))
      );
    };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4 overflow-auto">
      <div className="bg-white w-full max-w-3xl rounded-lg shadow-lg p-5 relative max-h-[90vh] overflow-y-auto">
        ...
        {/* Route & Date */}
        <div className="mb-4">
          <p><b>Route:</b> {trip.route.from} → {trip.route.to}</p>
          <p><b>Date:</b> {new Date(trip.date).toLocaleDateString()}</p>
        ...
        <div className="mb-4 border p-3 rounded-lg">
          ...
          <p><b>Name:</b> {trip.driver.name}</p>
          <p><b>Phone:</b> {trip.driver.phone}</p>
          <p><b>License No:</b> {trip.driver.license_number}</p>
          <p><b>License Expiry:</b> {new Date(trip.driver.license_expiry_date).toLocaleDateString()}</p>
        </div>

        {/* Truck */}
        <div className="mb-4 border p-3 rounded-lg">
          <h3 className="font-semibold mb-2">Truck Details</h3>
          <p><b>Truck Number:</b> {trip.selectedTruck.truck_number}</p>
          <p><b>Capacity:</b> {trip.selectedTruck.capacity_kg} kg / {trip.selectedTruck.capacity_m3} m³</p>
        </div>
        ...
        <div className="mb-4">
          <p><b>Trip Capacity:</b> {trip.capacity_kg} kg / {trip.capacity_m3} m³</p>
        </div>
