...
const PriceListTemplateForm: React.FC<PriceListTemplateFormProps> = ({ template, onSave, onCancel }) => {
  ...
  useEffect(() => {
    ...
    setAvailableProducts((prev) => {
      const filtered = prev.filter((product) => !selectedProducts.some((p) => p.id === product.id))
      // Sort alphabetically
  ...
  const handleAddSelected = () => {
    ...
    setSelectedProducts((prev) => {
      ...
      return updated.sort((a, b) => (a.productName || a.name).localeCompare(b.productName || b.name))
    })

    // Remove from available products
    setAvailableProducts((prev) => {
      const filtered = prev.filter((product) => !selectedAvailableItems.includes(product.id))
      return filtered.sort((a, b) => a.name.localeCompare(b.name))
    })
  ...
  const handleRemoveSelected = () => {
    ...
    setSelectedProducts((prev) => {
      const filtered = prev.filter((product) => !selectedSelectedItems.includes(product.id))
      return filtered.sort((a, b) => (a.productName || a.name).localeCompare(b.productName || b.name))
    })
  ...
  useEffect(() => {
    if (template?.products) {
      const sorted = [...template.products].sort((a, b) => a.name.localeCompare(b.name))
  ...
  useEffect(() => {
    ...
    setAvailableProducts((prev) => [...prev].sort((a, b) => a.name.localeCompare(b.name)))
                  ...
                  {selectedProducts.map((product) => (
                    ...
                    >
                      ...
                      <TableCell className="text-center">
                        <button type="button" className="cursor-pointer" onClick={() => handleChnagePrice(product)}> Copy All</button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
