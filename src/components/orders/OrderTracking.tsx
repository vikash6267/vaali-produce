...
import { Search, MapPin, CheckCircle2, Truck, Package, Clock, Calendar } from 'lucide-react';
  ...
  const handleTrack = () => {
    ...
    setTimeout(() => {
      ...
      setTrackingResult({
        ...
        events: [
          { date: "2023-06-10", time: "08:15 AM", location: "Warehouse", status: "Order processed", completed: true },
          { date: "2023-06-11", time: "09:30 AM", location: "Sorting Center", status: "Shipment packed", completed: true },
          { date: "2023-06-12", time: "02:45 PM", location: "Distribution Center", status: "In transit", completed: true },
          { date: "2023-06-14", time: "10:00 AM", location: "Local Courier", status: "Out for delivery", completed: false },
          { date: "2023-06-15", time: "12:00 PM", location: "Destination", status: "Delivered", completed: false },
        ]
      });
    }, 1500);
  };
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <div className="font-medium">{trackingResult.recipient}</div>
                      </div>
                      <div>
                        <div className="text-sm text-muted-foreground">Shipping Address</div>
                        <div className="font-medium">{trackingResult.address}</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                      ...
                      <div className="space-y-4">
                        {trackingResult.events.map((event: any, index: number) => (
                          <div key={index} className="relative pl-6">
                            <div className={`absolute left-0 top-0 h-3.5 w-3.5 rounded-full border-2 ${
                              event.completed 
          ...
          {!trackingResult && !isTracking && (
            <div className="flex items-center justify-center p-8">
              <div className="text-center text-muted-foreground">
                <Package size={48} className="mx-auto mb-3 text-muted-foreground/60" />
                <p className="text-sm">Enter a tracking number to view shipment details</p>
              </div>
            </div>
          )}
        </CardContent>
