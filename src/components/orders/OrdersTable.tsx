...
const OrdersTable: React.FC<OrdersTableProps> = ({
  ...
  const [workOrderDialogOrder, setWorkOrderDialogOrder] =
  ...
  const filteredOrders = orders.filter(
    (order) =>
      ...
      (order.orderType === activeTab ||
        (!order.orderType && activeTab === "Regural"))
  );
    ...
    return (
      <InvoiceGenerator
        ...
        onViewClientProfile={() =>
          ...
          handleViewClientProfile(selectedOrder.clientId)
        }
      />
    );
  };
  ...
  const handleDownloadMergedProducts = (type: string) => {
    ...
    const dateRangeText = `${type} Orders (${formatDate(
      oldestDate
  ...
  return (
    <div className="space-y-4 animate-slide-up">
      ...
      <div className="flex justify-between items-center border-b">
        ...
        </div>
        {
          <Button
            ...
            variant="outline"
            className="mb-2 mr-2"
            onClick={() => handleDownloadMergedProducts(activeTab)}
          >
                  ...
                  <TableCell>
                    <div className="flex items-center">
                      <Link
                        ...
                        onClick={() =>
                          order.clientId &&
                          handleViewClientProfile(order.clientId)
                        }
                      >
                        {order.clientName}
                      </Link>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuSub>
                          ...
                          <DropdownMenuSubContent className="min-w-[220px]">
                            <DropdownMenuItem
                              onClick={() =>
                                handleCreateDocument(order, "invoice")
                              }
                            >
                            ...
                            {(!order.orderType ||
                              order.orderType === "Regural") && (
                              <>
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleCreateDocument(order, "transport")
                                  }
                                >
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleCreateDocument(order, "delivery")
                                  }
                                >
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleCreateDocument(order, "custom")
                                  }
                                >
                                ...
                                >
                                  <Wrench className="mr-2 h-4 w-4" /> Create
                                  Work Order
                                </DropdownMenuItem>
                              </>
                            )}
      ...
      <Dialog
        open={!!workOrderDialogOrder}
        onOpenChange={(open) => !open && setWorkOrderDialogOrder(null)}
      >
