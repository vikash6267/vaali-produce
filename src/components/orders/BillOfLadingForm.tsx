...
import { 
  Truck, FileText, Download, Copy, Share2, CheckCircle2,
  Printer, Package, ClipboardList, Clipboard, X
} from 'lucide-react';
...
import { generateBillOfLadingPDF, BillOfLadingData, generateTransportationReceiptPDF } from '@/utils/exportDocuments';
...
const bolSchema = z.object({
  ...
  serviceLevel: z.enum(["Standard", "Expedited", "Same Day"]).default("Standard"),
});
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl">
        ...
        {!bolGenerated ? (
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleGenerateBOL)} className="space-y-4">
              <div className="grid md:grid-cols-2 gap-6">
                {/* Shipper Information */}
                <div className="space-y-4">
                  <h3 className="font-medium text-lg border-b pb-2">Shipper Information</h3>
                ...
                <div className="space-y-4">
                  <h3 className="font-medium text-lg border-b pb-2">Consignee Information</h3>
              ...
              <div className="space-y-4 pt-2">
                <h3 className="font-medium text-lg border-b pb-2">Carrier & Freight Information</h3>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-sm border-b pb-1">
                      ...
                      <span className="font-medium">{calculateTotalPieces()} units</span>
                    </div>
                    <div className="flex justify-between text-sm border-b pb-1">
                      <span>Estimated Weight:</span>
                      <span className="font-medium">{getTotalWeight()} lbs</span>
                    </div>
                    <div className="flex justify-between text-sm border-b pb-1">
                      <span>NMFC Code:</span>
          ...
          <>
            <div className="p-6 border rounded-md bg-white space-y-6">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-xl font-bold text-gray-800">BILL OF LADING</h2>
                  <p className="text-sm text-gray-600 mt-1">ORIGINAL - NOT NEGOTIABLE</p>
                  <div className="flex items-center gap-1 mt-1">
                    <p className="text-sm text-gray-600">B/L Number: {bolNumber}</p>
                ...
                <div className="text-right">
                  <div className="border border-dashed border-gray-400 p-2 inline-block">
                    <p className="text-xs text-gray-600">For Carrier Use Only</p>
                    <p className="text-xs">Trailer #: {form.getValues().trailerNumber}</p>
                    <p className="text-xs">Seal #: {form.getValues().sealNumber}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="border p-3 rounded-md bg-gray-50">
                  <h3 className="font-medium text-sm mb-2 uppercase text-gray-700">Shipper</h3>
                  <p className="font-medium">{form.getValues().shipperName}</p>
                  <p>{form.getValues().shipperAddress}</p>
                  <p>{form.getValues().shipperCity}, {form.getValues().shipperState} {form.getValues().shipperZip}</p>
                </div>
                
                <div className="border p-3 rounded-md bg-gray-50">
                  <h3 className="font-medium text-sm mb-2 uppercase text-gray-700">Consignee</h3>
                  <p className="font-medium">{form.getValues().consigneeName}</p>
                  <p>{form.getValues().consigneeAddress}</p>
                  <p>{form.getValues().consigneeCity}, {form.getValues().consigneeState} {form.getValues().consigneeZip}</p>
                </div>
              </div>
              
              <div className="border p-3 rounded-md bg-gray-50">
                <h3 className="font-medium text-sm mb-2 uppercase text-gray-700">Carrier Information</h3>
                <div className="grid md:grid-cols-2 gap-2">
                  <div>
                    <p><span className="text-gray-600">Carrier Name:</span> {form.getValues().carrierName}</p>
                    <p><span className="text-gray-600">Service Level:</span> {form.getValues().serviceLevel}</p>
                  </div>
                  <div>
                    <p><span className="text-gray-600">Freight Terms:</span> {form.getValues().freightTerms}</p>
                    <p><span className="text-gray-600">Order Reference:</span> {order.id}</p>
                  </div>
                </div>
              </div>
              
              <div>
                <h3 className="font-medium text-sm mb-2 uppercase text-gray-700">Freight Description</h3>
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {order.items.map((item, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell className="text-right">{item.quantity * 2}</TableCell>
                        <TableCell className="text-right">157250</TableCell>
                        <TableCell>50</TableCell>
                      </TableRow>
                    ))}
                    <TableRow>
                      <TableCell className="font-medium">{calculateTotalPieces()}</TableCell>
                      <TableCell>Total</TableCell>
                      <TableCell className="text-right font-medium">{getTotalWeight()}</TableCell>
                      <TableCell></TableCell>
                      <TableCell></TableCell>
                    </TableRow>
                  </TableBody>
              ...
              {form.getValues().specialInstructions && (
                <div className="border p-3 rounded-md bg-gray-50">
                  <h3 className="font-medium text-sm mb-1 uppercase text-gray-700">Special Instructions</h3>
              ...
              <div className="grid md:grid-cols-2 gap-6 pt-4 border-t">
                <div>
                  <h3 className="font-medium text-sm mb-2 uppercase text-gray-700">Shipper Certification</h3>
                  <p className="text-sm">
                    This is to certify that the above named materials are properly classified, described, packaged, marked and labeled, and are in proper condition for transportation according to the applicable regulations.
                  </p>
                  <div className="mt-4">
                    <p className="font-medium">{form.getValues().signatureShipper}</p>
                    <p className="text-xs text-gray-600">Digital Signature</p>
                    <p className="text-xs text-gray-600">Date: {formatDate(new Date().toISOString())}</p>
                  </div>
                </div>
                <div>
                  <h3 className="font-medium text-sm mb-2 uppercase text-gray-700">Carrier Certification</h3>
                  <p className="text-sm">
                    Carrier acknowledges receipt of packages and required placards. Carrier certifies emergency response information was made available and/or carrier has the appropriate information and training.
                  </p>
                  <div className="mt-4">
                    <p className="text-sm italic text-gray-500">Awaiting carrier signature</p>
                  </div>
                </div>
              </div>
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="destructive" size="icon" onClick={handleNewBOL}>
                        <X className="h-4 w-4" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
